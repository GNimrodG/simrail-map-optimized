...
const SignalMarkerPopup: FunctionComponent<SignalMarkerPopupProps> = ({
    ...
    const showSignalLines = useCallback(
        async (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
            ...
            try {
                if (includeDirect) {
                    // These are the direct lines with only 2 coords
                ...
                if (includeConnections) {
                    // These are the lines that are fetched from the server using the route points
                    const {lines} = await calculateSignalConnectionLines({
                        signal,
                        signalsData,
                        getLinesForConnection: dataProvider.getLinesForSignalConnection.bind(dataProvider),
                        startIndex: nextIndex,
                    });

                    combinedLines = includeDirect
                        ? [
                            // Filter out straight lines if we have real lines for them 
                            ...combinedLines.filter(x => !lines.some(y => y.label === x.label)),
                            ...lines
                        ]
                        : lines;
                    setMapLines({signal: signal.Name, lines: combinedLines});
                        ...
                        <Typography textAlign="center">
                            <Trans
                                ...
                                components={[
                                    ...
                                    <Typography key="train-velocity"
                                                color={getSpeedColorForSignal(trains[0].TrainData.Velocity)}/>,
                                    <Typography
                                        key="train-distance"
                ...
                {signal.NextSignalWithTrainAhead && (
                    <Typography textAlign="center">
                        <Trans
                            ...
                            components={[
                                <Chip key="NextSignalWithTrainAhead-signal"
                                      onClick={handleNextSignalWithTrainAheadClick}/>,
                            ...
                            {signal.PrevSignals.map((s) => (
                                <Chip
                                    ...
                                    endDecorator={
                                        ...
                                        <ChipDelete onDelete={prevSignalDeleteHandlers[s.Name]}/>
                                    }>
                        ...
                        <Stack direction="row" alignItems="center" justifyContent="center" gap={0.5} flexWrap="wrap">
                            ...
                            {signal.NextSignals.map((s) => (
                                <Chip
                                    ...
                                    endDecorator={
                                        localStorage.getItem("adminPassword") &&
                                        <ChipDelete onDelete={nextSignalDeleteHandlers[s.Name]}/>
                                    }>
                                    {s.Name} ({s.Vmax != null ? (s.Vmax === 32767 ? "VMAX" : s.Vmax + " km/h") : "?"})
                                </Chip>
                            ))}
                                ...
                                title={
                                    <>
                                        <Typography>
                                            ...
                                            <Typography
                                                sx={{color: "orange"}}>{t("Orange")}</Typography>: {t("PreviousSignals")}
                                        </Typography>
                                        <Typography>
                                            <Typography sx={{color: "blue"}}>{t("Blue")}</Typography>/
                                            <Typography
                                                sx={{color: "purple"}}>{t("Purple")}</Typography>: {t("NextSignals")}
                                        </Typography>
                                    </>
                                }>
                                <Chip
